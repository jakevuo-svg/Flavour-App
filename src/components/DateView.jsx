...
const DateView = ({ events = [], onEventClick, onAddEvent }) => {
  ...
  return (
    <div style={{ ...S.border, ...S.bg, borderTop: "none", padding: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2 }}>
        {FINNISH_MONTHS.map((monthName, monthIndex) => (
          <div
            ...
            style={{
              border: expandedMonth === monthIndex ? '2px solid var(--c-border)' : '2px solid var(--c-border-row)',
              background: expandedMonth === monthIndex ? 'var(--c-bg-hover)' : 'var(--c-bg-card)',
              padding: 15,
              cursor: 'pointer',
              textAlign: 'center',
            }}
          >
            ...
            <div style={{ color: 'var(--c-text-muted)', fontSize: 11, marginTop: 4 }}>{selectedYear}</div>
          </div>
        ))}
      </div>
      ...
      {expandedMonth !== null && (
        <div style={{ border: '2px solid var(--c-border)', borderTop: 'none', background: 'var(--c-bg-card)', padding: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
            {['MA', 'TI', 'KE', 'TO', 'PE', 'LA', 'SU'].map(day => (
              <div key={day} style={{ textAlign: 'center', fontSize: 10, fontWeight: 700, color: 'var(--c-text-muted)', padding: 6, letterSpacing: 1 }}>
                {day}
              </div>
            ))}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    padding: 8,
                    background: isSelected ? 'var(--c-accent-bg)' : isToday ? 'var(--c-bg-hover)' : 'transparent',
                    border: isSelected ? '2px solid var(--c-border)' : '1px solid var(--c-border-row)',
                    cursor: 'pointer',
                    fontSize: 13,
          ...
          {selectedDay && selectedDay.month === expandedMonth && (
            <div style={{ marginTop: 20, borderTop: '2px solid var(--c-border)', paddingTop: 16 }}>
              ...
              {selectedDayEvents.length === 0 ? (
                <div style={{ color: 'var(--c-text-muted)', fontSize: 12 }}>Ei tapahtumia tälle päivälle</div>
              ) : (
